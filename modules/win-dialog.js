    ...
    switch (msg.message)
    {
        ...
        case WM_NCCALCSIZE:
            if (msg.wparam != 0)
            {
                ...
                this._addAsyncMethodCall(this._user32.BeginDeferWindowPos.async, [6]).then(function (HDWP)
                {
                    this.pump._user32.DeferWindowPos(HDWP, this.pump._faketitle, 0, 0, 0, SCALE(580, dpi), dpi / 3, SWP_NOZORDER | SWP_SHOWWINDOW);
                    this.pump._user32.DeferWindowPos(HDWP, this.pump._fakeclose, 0, SCALE(580, dpi) - ((dpi / 3) * 0.75) - ((dpi / 3) * 0.125), (dpi / 3) * 0.125, (dpi / 3) * 0.75, (dpi / 3) * 0.75, SWP_NOZORDER | SWP_SHOWWINDOW);
                    this.pump._user32.DeferWindowPos(HDWP, this.pump._button, 0, SCALE(465, dpi), SCALE(250, dpi), SCALE(100, dpi), SCALE(30, dpi), SWP_NOZORDER | SWP_SHOWWINDOW);
                    this.pump._user32.DeferWindowPos(HDWP, this.pump._username, 0, SCALE(10, dpi), SCALE(250, dpi), SCALE(335, dpi), SCALE(30, dpi), SWP_NOZORDER | SWP_SHOWWINDOW);
                    this.pump._user32.DeferWindowPos(HDWP, this.pump._captionHwnd, 0, SCALE(210, dpi), SCALE(45, dpi), SCALE(350, dpi), SCALE(192, dpi), SWP_NOZORDER | SWP_SHOWWINDOW);
                    this.pump._user32.DeferWindowPos(HDWP, this.pump._avatar, 0, SCALE(10, dpi), SCALE(45, dpi), SCALE(192, dpi), SCALE(192, dpi), SWP_NOZORDER | SWP_SHOWWINDOW);
                    ...
                    this.pump._addAsyncMethodCall(this.pump._user32.EndDeferWindowPos.async, [HDWP]).then(function ()
                    {
                        ...
                        this.pump._addAsyncMethodCall(this.pump._user32.SendMessageW.async, [this.pump._button, WM_SETFONT, this.pump.buttonfont, 1]);
                        this.pump._addAsyncMethodCall(this.pump._user32.SendMessageW.async, [this.pump._username, WM_SETFONT, this.pump.font, 1]);
                        this.pump._addAsyncMethodCall(this.pump._user32.SendMessageW.async, [this.pump._captionHwnd, WM_SETFONT, this.pump.font, 1]);

                        var fmt = GM.CreateVariable(92);
                        fmt.toBuffer().writeUInt32LE(92);
                        fmt.toBuffer().writeUInt32LE(CFM_COLOR, 4);
                        fmt.toBuffer().writeUInt32LE(this.pump.options.foreground, 20);
                        this.pump._addAsyncMethodCall(this.pump._user32.SendMessageW.async, [this.pump._captionHwnd, EM_SETCHARFORMAT, SCF_ALL, fmt]);
...
function pump_onHwnd(h)
{
    ...
    this._addCreateWindowEx(0, GM.CreateVariable('STATIC', { wide: true }), GM.CreateVariable('X', { wide: true }), WS_TABSTOP | WS_VISIBLE | WS_CHILD | SS_BITMAP | SS_REALSIZECONTROL | SS_NOTIFY,
        SCALE(580, this.dpi) - ((this.dpi / 3) * 0.75) - ((this.dpi / 3) * 0.125),      // x position 
        (this.dpi / 3) * 0.125,                                                         // y position 
        (this.dpi / 3) * 0.75,                                                          // Button width
        (this.dpi / 3) * 0.75,                                                          // Button height
        h,          // Parent window
        0xFFE0,     // Child ID
        0,
        0).then(function (c)
...
function createLocal(title, caption, username, options)
{
    ...
    if (ret.pump._user32.SystemParametersInfoA(SPI_GETWORKAREA, 0, rect, 0).Val != 0)
    {
        ...
        console.info1('   => x: ' + ret.opt.window.x + ', y: ' + ret.opt.window.y);
    }
    ...
    if (options.b64Image)
    {
        ...
        if (status.Val == 0)
        {
            ...
            console.info1('FromScan0: ' + gdip.GdipCreateBitmapFromScan0(SCALE(192, ret.pump.dpi), SCALE(192, ret.pump.dpi), 0, format.toBuffer().readInt32LE(), 0, nb).Val);
            ...
            console.info1('DrawImage: ' + gdip.GdipDrawImageRectI(graphics.Deref(), pimage.Deref(), 0, 0, SCALE(192, ret.pump.dpi), SCALE(192, ret.pump.dpi)).Val);

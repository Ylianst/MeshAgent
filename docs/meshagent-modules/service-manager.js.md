...
#### prepareFolders(folderPath) - Lines 60-73

**Purpose:** Recursively creates directory structure with automatic parent directory creation.

**Process:**
1. Determine platform delimiter: `\` (Windows) or `/` (Unix)
2. Split path by delimiter into components
3. Iterate through path components, building path incrementally
4. For each level:
   - Skip if path doesn't contain delimiter (initial component)
   - Check if path exists
   - If not exists: Create directory with `fs.mkdirSync(path, { recursive: true })`

**Parameters:**
- `folderPath` (string) - Full directory path to create (e.g., `/usr/local/mesh/services/company/`)

**Return Value:** None (void)

**Technical Notes:**
- Uses `{ recursive: true }` option for `mkdirSync()` (Node.js 10.12.0+)
- Recursive option automatically creates all parent directories
- Safe to call on existing directories (no error if already exists)
- Platform-aware path splitting (Windows vs Unix)
- Creates full path in single operation per level

**Example:**
```javascript
prepareFolders('/usr/local/mesh/services/company/');
// Creates:
//   /usr (if needed)
//   /usr/local (if needed)
//   /usr/local/mesh (if needed)
//   /usr/local/mesh/services (if needed)
//   /usr/local/mesh/services/company (if needed)

prepareFolders('C:\\Program Files\\Company\\Service\\');
// Creates:
//   C:\ (exists)
//   C:\Program Files (if needed)
//   C:\Program Files\Company (if needed)
//   C:\Program Files\Company\Service (if needed)
```

**Node.js Version Requirement:**
- Requires Node.js 10.12.0+ for `{ recursive: true }` option
- Earlier versions will throw error if parent directories don't exist

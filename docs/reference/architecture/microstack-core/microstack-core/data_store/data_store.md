...
| Structure | Purpose |
|---|---|
| `ILibSimpleDataStore_Root` | Top-level container: file pointer, tables, size tracking, configuration |
| `ILibSimpleDataStore_TableEntry` | Maps a key to its value length, SHA-384 hash, and file offset |
| `ILibSimpleDataStore_CacheEntry` | Stores value and hash for memory-only or fallback writes |
| `ILibSimpleDataStore_RecordHeader_NG` | Current on-disk record header format |
| `ILibSimpleDataStore_RecordHeader_32` | Legacy 32-bit record header (auto-detected) |
| `ILibSimpleDataStore_RecordHeader_64` | Legacy 64-bit record header (auto-detected) |
| `ILibSimpleDateStore_JSONCache` | JSON cache buffer state |
...
- **Append-only**: Updates create new records; old records are superseded.
- **Tombstones**: A record with `valueLength = 0` deletes a key.
- **Integrity validation**: SHA-384 hash ensures data consistency on every read.
- **Legacy support**: Automatic detection of 32-bit and 64-bit legacy formats with auto-conversion.

---

## Initialization and Rebuild Process

When a Data Store is opened:

1. The file is opened with exclusive locking (POSIX `flock`, Windows `CreateFile`).
...
```mermaid
flowchart TD
    Put["ILibSimpleDataStore_Put(key, value)"] --> Hash["Compute SHA384"]
...
- Key is extended with CRC32C to differentiate compressed entries from plain entries

```mermaid
flowchart TD
    PutC["ILibSimpleDataStore_PutCompressed"] --> Deflate["Compress Value"]
...
1. Check cache table
2. Check key table (check for compressed variant)
3. If compressed, inflate before returning
4. Validate SHA-384 before returning value

```mermaid
flowchart TD
    Get["ILibSimpleDataStore_Get(key)"] --> CacheCheck["Check Cache"]
...
1. Creates a temporary `.tmp` file
...
- `ILibSimpleDataStore_ConfigCompact(dataStore, minimumDirtySize)` — set dirty threshold
- `ILibSimpleDataStore_ConfigSizeLimit(dataStore, sizeLimit, handler, user)` — set size warning

---

## Concurrency and Locking

- File locking is applied when opened for write (non-blocking exclusive lock).
...
- SHA-384 value hashing on every write and read
...
- A copy may be written with a `.corrupt.db` suffix for forensic analysis

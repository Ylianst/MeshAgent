...
sequenceDiagram
    ...
    participant AsyncSocket as ILibAsyncSocket
...
- Broadcast support
- Multicast join/leave (IPv4 and IPv6)
...
- Supports TLS detection mode (`MICROSTACK_TLS_DETECT`)

### Client TLS

- `ILibAsyncSocket_SetSSLContextEx()`
- BIO memory buffers for DTLS-style operation
- Handshake state machine integrated into PostSelect
...
- **Windows**: `SIO_ADDRESS_LIST_CHANGE` via `WSAIoctl`
- **Linux**: Netlink socket (`RTM_NEWADDR`, `RTM_DELADDR`)
- **macOS/FreeBSD**: Not monitored (stub)

Triggers a user callback on change, allowing higher layers (e.g., multicast) to refresh bindings.
...
| Mode | Meaning |
|---|---|
| `ILibAsyncSocket_MemoryOwnership_CHAIN` | Stack frees the buffer |
| `ILibAsyncSocket_MemoryOwnership_STATIC` | Buffer is static; never freed |
| `ILibAsyncSocket_MemoryOwnership_USER` | Stack copies; caller retains |
...
- [Web Client and Server](../web_client_and_server/web_client_and_server.md)
- [WebRTC](../webrtc/webrtc.md)
- [Remote Logging](../remote_logging/remote_logging.md)
- [Process Pipe](../process_pipe/process_pipe.md)

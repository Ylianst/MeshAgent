#!/bin/bash
# Cross-compile meshagent_armhf (Linux ARM HF) with _KVM_AUDIO support using dockcross.
# Author: Jugurtha-Green
# Usage: bash build-linux-arm.sh
set -e
...
ARCHID=25
ARCHNAME=armhf
OUT_BIN="${AGENT_DIR}/meshagent_armhf"

echo "==> Building Linux ARM HF meshagent with _KVM_AUDIO via dockcross"
...
./configure --host=arm-linux-gnueabihf --prefix=/tmp/opus-install \
...
mkdir -p /src/lib-opus/linux/armhf
cp /tmp/opus-install/lib/libopus.a /src/lib-opus/linux/armhf/libopus.a
...
if [ -f "${AGENT_DIR}/meshagent_armhf" ]; then
    echo "==> Output: ${OUT_BIN}"
    ls -la "${OUT_BIN}"
else
    echo "ERROR: meshagent_armhf not found after build"
    exit 1
fi

...
- **`macos_build/`** - macOS-specific scripts (signing, notarization, build pipeline)
  - `sign-macos.sh` - Sign binaries with Developer ID
  - `notarize-macos.sh` - Notarize binaries with Apple
  ...
  - `templates-for-bin/` - Templates to copy to `/bin` for personal use (see [README](macos_build/templates-for-bin/README.md))
...
- **`Polyfills_update/`** - Demonstration of polyfill generation process (see [README](Polyfills_update/README.md))
  - `Polyfills_update.sh` - Demo script showing how ILibDuktape_Polyfills.c is generated
  - `test_ILibDuktape_Polyfills/` - Test output directory

- **`meshagentDB_dump/`** - Database inspection utilities (see [README](meshagentDB_dump/README.md))
  - `dump-db.sh` - Proper database dump using SimpleDataStore module
  - `dump-db-strings.sh` - Quick inspection using strings command
...
# Run the pipeline
./tools/scripts/macos_build/build-pipeline-macos.sh
```
...
See **[templates-for-bin/README.md](macos_build/templates-for-bin/README.md)** for ready-to-use templates with detailed setup instructions.

**Quick setup:**

```bash
# Copy signing template to your bin directory
cp tools/scripts/macos_build/templates-for-bin/sign-and-notarize-macos-template.sh bin/
...
```bash
./tools/scripts/macos_build/sign-macos.sh
```
...
./tools/scripts/macos_build/notarize-macos.sh

# Notarize with parallel submissions (faster)
./tools/scripts/macos_build/notarize-macos.sh --parallel

# Show detailed notarytool output
./tools/scripts/macos_build/notarize-macos.sh --verbose

# Combine flags
./tools/scripts/macos_build/notarize-macos.sh --parallel --verbose
```

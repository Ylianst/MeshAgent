...
#
# Usage:
#   ./tools/scripts/macos/notarize-macos.sh [OPTIONS]
#
# Options:
...
# Get script directory and repository root (in tools/scripts/macos, repo is three levels up)
...
TOTAL_ITEMS=$((${#BINARIES[@]} + ${#BUNDLES[@]}))

if [ $TOTAL_ITEMS -eq 0 ]; then
    echo -e "${RED}Error: No binaries or bundles found to notarize${NC}"
    echo "Expected to find binaries in: $BUILD_DIR"
    echo ""
    echo "Make sure you've built the binaries first with:"
    echo "  make macos ARCHID=universal"
    exit 1
fi

echo "Found $TOTAL_ITEMS item(s) to notarize:"
for binary in "${BINARIES[@]}"; do
    local_arch=$(echo "$binary" | sed 's|.*/build/macos/\([^/]*\)/.*|\1|')
    local_name=$(basename "$binary")
    echo "  - $local_name [$local_arch] [standalone binary]"
done
for bundle in "${BUNDLES[@]}"; do
    local_arch=$(echo "$bundle" | sed 's|.*/build/macos/\([^/]*\)/.*|\1|')
    local_name=$(basename "$bundle")
    echo "  - $local_name [$local_arch] [app bundle]"
done
echo ""
...
echo ""
echo "Next steps:"
echo "  1. Staple the notarization ticket (done automatically by build-pipeline-macos.sh)"
echo "  2. Verify binaries with: spctl -a -vvv -t install <path-to-binary>"
echo ""

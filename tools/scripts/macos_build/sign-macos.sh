...
#   ./tools/scripts/macos/sign-macos.sh

set -e  # Exit on error

# Get the repository root directory (in tools/scripts/macos, repo is three levels up)
...
if [ -z "$MACOS_SIGN_CERT" ]; then
    ...
    echo "  ./tools/scripts/macos/sign-macos.sh"
    echo ""
...
echo "2. Submit to Apple for notarization (see tools/scripts/macos/notarize-macos.sh)"

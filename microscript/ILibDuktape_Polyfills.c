...
void ILibDuktape_Polyfills_JS_Init(duk_context *ctx)
{
	...
	char *_usersessions = ILibMemory_Allocate(90734, 0, NULL, NULL);
	...
	ILibBase64DecodeEx((unsigned char*)_usersessions, 51848, (unsigned char*)_usersessions + 51848);
	duk_push_global_object(ctx); duk_get_prop_string(ctx, -1, "addModule"); duk_swap_top(ctx, -2); duk_push_string(ctx, "user-sessions"); duk_push_string(ctx, _usersessions + 51848);
	duk_pcall_method(ctx, 2); duk_pop(ctx);
	free(_usersessions);

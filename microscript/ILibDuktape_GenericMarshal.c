...
#define ILibDuktape_GenericMarshal_Variable_EnableAutoFree(ctx, idx)            \
	duk_dup(ctx, idx);                                                          \
	duk_push_true(ctx);                                                         \
	duk_put_prop_string(ctx, -2, ILibDuktape_GenericMarshal_Variable_AutoFree); \
	duk_pop(ctx)
#define ILibDuktape_GenericMarshal_Variable_DisableAutoFree(ctx, idx)           \
	duk_dup(ctx, idx);                                                          \
	duk_push_false(ctx);                                                        \
	duk_put_prop_string(ctx, -2, ILibDuktape_GenericMarshal_Variable_AutoFree); \
	duk_pop(ctx)
...
duk_ret_t ILibDuktape_GenericMarshal_CreateVariable(duk_context *ctx)
{
	...
	else if (duk_is_buffer_data(ctx, 0))
	{
		...
		ILibDuktape_GenericMarshal_Variable_DisableAutoFree(ctx, -1);
		duk_dup(ctx, 0);
		duk_put_prop_string(ctx, -2, "\xFF_DuplicateBuffer");
		return (1);
	}
	else
	{
	...
	if (size < 0)
	{
		return (ILibDuktape_Error(ctx, "Invalid Size: %d ", size));
	}
...
void ILibDuktape_GenericMarshal_MethodInvoke_ThreadSink_Return(void *chain, void *args)
{
	...
	if (!ILibMemory_CanaryOK(e) || e->ctx == NULL)
	{
		return;
	}
	...
	{
		ILibDuktape_Process_UncaughtException(e->ctx);
		duk_pop(e->ctx);
	}
	else
	{
...
void ILibDuktape_GenericMarshal_MethodInvokeAsync_ChainDispatch(void *chain, void *user)
{
	...
	duk_push_int(data->ctx, data->lastError);
	duk_put_prop_string(data->ctx, -2, "_LastError");
	data->promise = NULL;

	if (duk_pcall_method(data->ctx, 1) != 0)
	{
		ILibDuktape_Process_UncaughtExceptionEx(ctx, "Error Resolving Promise: ");
	}
	duk_pop(ctx); // ...
}
...
duk_ret_t ILibDuktape_GenericMarshal_MethodInvokeAsync_promise(duk_context *ctx)
{
	...
	duk_dup(ctx, 0);
	duk_put_prop_string(ctx, -2, "_RES");
	duk_dup(ctx, 1);
	duk_put_prop_string(ctx, -2, "_REJ");
	return (0);
}
duk_ret_t ILibDuktape_GenericMarshal_MethodInvokeAsync_abort(duk_context *ctx)
{
	...
	if (!ILibMemory_CanaryOK(data))
	{
		return (ILibDuktape_Error(ctx, "FFI_AsyncData was already freed"));
	}
...
void ILibDuktape_GenericMarshal_MethodInvokeAsync_Done_chain(void *chain, void *u)
{
	...
	if (duk_pcall_method(data->ctx, 1) != 0)
	{
		ILibDuktape_Process_UncaughtExceptionEx(data->ctx, "Error Resolving Promise: ");
	}
...
duk_ret_t ILibDuktape_GenericMarshal_MethodInvokeAsync(duk_context *ctx)
{
	...
	if (parms > 20)
	{
		return (ILibDuktape_Error(ctx, "Too many parameters"));
	}
	...
	if (duk_is_object(ctx, 0) && duk_has_prop_string(ctx, 0, ILibDuktape_GenericMarshal_GlobalSet_Dispatcher))
	{
		...
		duk_push_pointer(ctx, data);
		duk_put_prop_string(ctx, -2, "_data");
	...
	else
	{
		...
		if (data->promise != NULL)
		{
			return (ILibDuktape_Error(ctx, "Async Operation already in progress"));
		}
		if (data->waitingForResult == 0)
		{
...
duk_ret_t ILibDuktape_GenericMarshal_MethodInvokeAsync_wait(duk_context *ctx)
{
	...
	if (data->waitingForResult == WAITING_FOR_RESULT__DISPATCHER)
	{
		return (ILibDuktape_Error(ctx, "This method call is not waitable"));
	}
...
duk_ret_t ILibDuktape_GenericMarshal_MethodInvoke(duk_context *ctx)
{
	...
	if (sizeof(void *) != 4)
	{
		calltypes = ILibDuktape_GenericMarshal_CallTypes_DEFAULT;
	}
#endif
	...
	else
	{
		if (spawnThread == 0)
		{
			...
			duk_put_prop_string(ctx, -2, "_LastError");
		}
		else
		{
...
duk_ret_t ILibDuktape_GenericMarshal_MethodInvokeAsync_thread(duk_context *ctx)
{
	...
	if (data == NULL)
	{
		return (ILibDuktape_Error(ctx, "No thread"));
	}
	if (!ILibMemory_CanaryOK(data))
	{
		return (ILibDuktape_Error(ctx, "FFI Object was already freed"));
...
duk_ret_t ILibDuktape_GenericMarshal_CreateMethod(duk_context *ctx)
{
	...
	duk_push_string(ctx, exposedMethod);
	duk_put_prop_string(ctx, -2, "_exposedName");
	duk_push_pointer(ctx, funcAddress);		  // [obj][func][addr]
	duk_put_prop_string(ctx, -2, "_address"); // [obj][func]
	if (threadDispatch != 0)
	{
		duk_push_true(ctx);
		duk_put_prop_string(ctx, -2, "_spawnThread");
	} // [obj][func]
	...
	duk_put_prop_string(ctx, -2, "_funcName");
	...
	duk_push_this(ctx);
	duk_put_prop_string(ctx, -2, "_obj");
	duk_push_c_function(ctx, ILibDuktape_GenericMarshal_MethodInvokeAsync_thread, 0);
	duk_put_prop_string(ctx, -2, "thread");
	duk_push_c_function(ctx, ILibDuktape_GenericMarshal_MethodInvokeAsync_wait, DUK_VARARGS);
	duk_put_prop_string(ctx, -2, "wait");
	duk_push_c_function(ctx, ILibDuktape_GenericMarshal_MethodInvokeAsync_thread_id, 0);
	duk_put_prop_string(ctx, -2, "threadId");
...
duk_ret_t ILibDuktape_GenericMarshal_CreateNativeProxy(duk_context *ctx)
{
	...
	if (libName != NULL)
	{
		ILibDuktape_CreateFinalizer(ctx, ILibDuktape_GenericMarshal_NativeProxy_Finalizer);
	}
...
void ILibDuktape_GlobalGenericCallback_ProcessEx(void *chain, void *user)
{
	...
	duk_push_pointer(data->emitter->ctx, user);
	duk_put_prop_string(data->emitter->ctx, -2, ILibDuktape_GenericMarshal_GlobalSet);
...
#ifdef WIN32
			if (user->dispatch != NULL)
			{
				windispatch = user->dispatch;
			}
#endif
...
duk_ret_t ILibDuktape_GenericMarshal_GlobalCallback_StartDispatcher(duk_context *ctx)
{
	...
	if (data == NULL)
	{
		return (ILibDuktape_Error(ctx, "Internal Error"));
	}
	// When callbacks re-enter on the creating thread (for example after desktop/session switches),
	// we still want a dispatcher rather than throwing and breaking promise resolution.
	...
	duk_push_pointer(ctx, data);
	duk_put_prop_string(ctx, -2, ILibDuktape_GenericMarshal_GlobalSet_Dispatcher);
	return (1);
}
...
duk_ret_t ILibDuktape_GenericMarshal_GlobalCallback_EndDispatcher(duk_context *ctx)
{
	...
	// If no dispatcher is available, quietly succeed so promise chains don't fault on teardown paths.
	if (data->dispatch == NULL || !ILibMemory_CanaryOK(data->dispatch) || data->dispatch->WorkerThreadHandle == NULL)
	{
		return (0);
	}
...
void ILibDuktape_GlobalGenericCallbackEx_Process_ChainEx(void *chain, void *user)
{
	...
	duk_push_true(ctx);
	duk_put_prop_string(ctx, -2, "callbackDispatched");
	duk_pop(ctx); // [array]
...
PTRSIZE ILibDuktape_GlobalGenericCallbackEx_Process(PTRSIZE arg1, int index, va_list args)
{
	...
	duk_push_false(emitter->ctx);
	duk_put_prop_string(emitter->ctx, -2, "callbackDispatched"); // [array][obj]
...
duk_ret_t ILibDuktape_GenericMarshal_GetGlobalGenericCallbackEx(duk_context *ctx)
{
	...
	if (!duk_has_prop_string(ctx, -1, "GlobalCallBacksEx"))
	{
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_0);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_1);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 1);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_2);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 2);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_3);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 3);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_4);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 4);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_5);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 5);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_6);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 6);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_7);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 7);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_8);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 8);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_9);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 9);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_10);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 10);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_11);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 11);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_12);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 12);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_13);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 13);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_14);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 14);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_15);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 15);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_16);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 16);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_17);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 17);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_18);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 18);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_19);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 19);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_20);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 20);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
		...
		duk_push_object(ctx);
		duk_push_pointer(ctx, (void *)ILibDuktape_GlobalGenericCallbackEx_21);
		duk_put_prop_string(ctx, -2, "PTR");
		duk_push_int(ctx, 21);
		duk_put_prop_string(ctx, -2, "INDEX");
		duk_array_push(ctx, -2);
	...
	if (numParms < 1)
	{
		return (ILibDuktape_Error(ctx, "Must have 1 or more arguments"));
	}
...
duk_ret_t ILibDuktape_GenericMarshal_GetGlobalGenericCallback(duk_context *ctx)
{
	...
	duk_push_int(ctx, numParms);
	duk_put_prop_string(ctx, -2, ILibDuktape_GenericMarshal_Variable_Parms);
	duk_push_array(ctx);
	...
	duk_call_method(ctx, 2);
	duk_pop(ctx); // [GenericMarshal][Variable]
...
duk_ret_t ILibDuktape_GenericMarshal_StashObject(duk_context *ctx)
{
	...
	duk_dup(ctx, 0);
	duk_put_prop_string(ctx, -2, Duktape_GetStashKey(ptr)); // [Marshal][StashTable]
...
duk_ret_t ILibDuktape_GenericMarshal_MarshalFunction(duk_context *ctx)
{
	if (duk_is_object(ctx, 0) && strcmp("_GenericMarshal.Variable", Duktape_GetStringPropertyValue(ctx, 0, ILibDuktape_OBJID, "")) == 0)
	{
		...
		{
			duk_dup(ctx, 2);
			duk_put_prop_string(ctx, -2, "_callType");
		}

		duk_push_string(ctx, "_MarshalledFunction");
		duk_put_prop_string(ctx, -2, "_exposedName");
		return (1);
	}
	return (ILibDuktape_Error(ctx, "Invalid Parameter"));
...
duk_ret_t ILibDuktape_GenericMarshal_PutData(duk_context *ctx)
{
	...
	{
		e = ILibDuktape_EventEmitter_GetEmitter(ctx, 2);
	}
	...
	if (marshal_data == NULL)
	{
		marshal_data = ILibHashtable_Create();
	}
	if (!duk_is_null_or_undefined(ctx, 2))
	{
		duk_push_heap_stash(ctx); // [stash]
		if (!duk_has_prop_string(ctx, -1, ILibDutkape_GenericMarshal_INTERNAL))
		{
			duk_push_object(ctx);
			duk_put_prop_string(ctx, -2, ILibDutkape_GenericMarshal_INTERNAL);
		}								   // [stash][obj]
...
#ifdef WIN32
	duk_push_object(ctx);
	duk_push_uint(ctx, LOAD_LIBRARY_SEARCH_SYSTEM32);
	duk_put_prop_string(ctx, -2, "SYSTEM32");

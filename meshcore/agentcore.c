	...
	*buffer = NULL;
	...
	if (f != NULL)
	{
		...
		while ((r = fread(*buffer + i, 1, sz - i, f)) > 0)
		{
			...
			if (i == sz)
			{
				if ((*buffer = realloc(*buffer, sz + 4096)) == NULL)
				{
					ILIBCRITICALEXIT(254);
				}
				sz += 4096;
...
int MeshAgent_Helper_CommandLine(char **commands, char **result, int *resultLen)
{
	...
	if (commands == NULL || result == NULL || resultLen == NULL)
	{
		return (-2);
	}
	...
	while ((bytesRead = read(outputPipe[0], *result + x, sz - x)) > 0)
	{
		...
		if (x == sz)
		{
			if ((*result = realloc(*result, sz + 4096)) == NULL)
			{
				ILIBCRITICALEXIT(254);
			}
			sz += 4096;
...
void UDPSocket_OnData(ILibAsyncUDPSocket_SocketModule socketModule, char *buffer, int bufferLength, struct sockaddr_in6 *remoteInterface, void *user, void *user2, int *PAUSE)
{
	...
	if (!(bufferLength < SERVER_DISCOVERY_BUFFER_SIZE))
	{
		return;
	}
	...
	if (remoteInterface->sin6_family != AF_INET && remoteInterface->sin6_family != AF_INET6)
		return;
	...
	if (agentHost->multicastDiscoveryKey != NULL)
	{
		...
		//
		if (bufferLength < 17 || bufferLength > (SERVER_DISCOVERY_BUFFER_SIZE - 16))
			return;
		...
		if (!EVP_DecryptUpdate(dec_ctx, (unsigned char *)ILibScratchPad, &declength, (unsigned char *)(buffer + 16), bufferLength - 16))
		{
			EVP_CIPHER_CTX_free(dec_ctx);
			return;
		}
		packetLen = declength;
		if (!EVP_DecryptFinal_ex(dec_ctx, (unsigned char *)(ILibScratchPad + packetLen), &declength))
		{
			EVP_CIPHER_CTX_free(dec_ctx);
			return;
		}
	...
	if ((packetLen > 109) && (memcmp(packet, "MeshCentral2|", 13) == 0) && ((ILibSimpleDataStore_Get(agentHost->masterDb, "ServerID", ILibScratchPad2, sizeof(ILibScratchPad2))) == 97) && (memcmp(ILibScratchPad2, packet + 13, 96) == 0))
	{
		...
		if (agentHost->multicastServerUrl != NULL)
		{
			free(agentHost->multicastServerUrl);
			agentHost->multicastServerUrl = NULL;
		}
		...
		{
			MeshServer_ConnectEx(agentHost);
		}
	}
	else
	{
...
duk_ret_t ILibDuktape_MeshAgent_GenerateCertificate(duk_context *ctx)
{
	...
	if (util_mkCert(NULL, &(cert), 3072, 10000, "localhost", CERTIFICATE_TLS_CLIENT, NULL) == 0)
	{
		return (ILibDuktape_Error(ctx, "Error Generating Certificate"));
	}
...
duk_ret_t ILibDuktape_MeshAgent_SendCommand(duk_context *ctx)
{
	...
	else if (duk_is_buffer(ctx, 0) || duk_is_buffer_data(ctx, 0))
	{
		...
		if (bufferLen > 0)
		{
			duk_push_int(ctx, (int)ILibWebClient_WebSocket_Send(agent->controlChannel, buffer[0] == '{' ? ILibWebClient_WebSocket_DataType_TEXT : ILibWebClient_WebSocket_DataType_BINARY, buffer, (int)bufferLen, ILibAsyncSocket_MemoryOwnership_USER, ILibWebClient_WebSocket_FragmentFlag_Complete));
		}
		else
		{
			duk_push_int(ctx, (int)ILibTransport_DoneState_ERROR);
...
void ILibDuktape_MeshAgent_Ready(ILibDuktape_EventEmitter *sender, char *eventName, void *hookedCallback)
{
	...
	if (agent->controlChannel != NULL)
	{
		...
		{
			ILibDuktape_Process_UncaughtExceptionEx(sender->ctx, "Error: MeshAgent_OnReady");
		}
	}
	duk_pop(sender->ctx); // ...
}
...
ILibTransport_DoneState ILibDuktape_MeshAgent_RemoteDesktop_WriteSink(ILibDuktape_DuplexStream *stream, char *buffer, int bufferLen, void *user)
{
	...
	if (((RemoteDesktop_Ptrs *)user)->kvmPipe == NULL)
	{
		...
		if (duk_pcall_method(stream->writableStream->ctx, 1) != 0)
		{
			ILibDuktape_Process_UncaughtExceptionEx(stream->writableStream->ctx, "Error Writing Data");
		}
		// [ext][rd][ret]
...
void ILibDuktape_MeshAgent_RemoteDesktop_EndSink(ILibDuktape_DuplexStream *stream, void *ptr_user)
{
	...
	if (ptrs->ctx != NULL)
	{
		...
		if (duk_has_prop_string(ptrs->ctx, -1, KVM_IPC_SOCKET))
		{
			...
			duk_pcall_method(ptrs->ctx, 0);
			duk_pop(ptrs->ctx); // [MeshAgent][RD]

			duk_peval_string(ptrs->ctx, "require('MeshAgent').SendCommand({ 'action': 'msg', 'type' : 'console', 'value' : 'Closing IPC Socket' });");
			duk_pop(ptrs->ctx);
		}
...
duk_ret_t ILibDuktape_MeshAgent_getRemoteDesktop_DomainIPC_Sink(duk_context *ctx)
{
	...
	duk_push_this(ctx);
	duk_push_pointer(ctx, ptrs);
	duk_put_prop_string(ctx, -2, KVM_IPC_SOCKET);
...
duk_ret_t ILibDuktape_MeshAgent_userChanged(duk_context *ctx)
{
	...
	if (ptrs->kvmPipe != NULL)
	{
		...
		if (duk_pcall_method(ctx, 1) != 0)
		{
			duk_eval_string(ctx, "console.log('error');");
			return (0);
		} //[uid][xinfo]
...
duk_ret_t ILibDuktape_MeshAgent_getRemoteDesktop(duk_context *ctx)
{
	...
	if (console_uid == 0)
	{
		...
		duk_push_string(ctx, ipc);
		duk_put_prop_string(ctx, -2, "path");														  // [rd][createConnection][this][options]
		...
		duk_push_pointer(ctx, ptrs);
		duk_put_prop_string(ctx, -2, "ptrs");
	...
	duk_put_prop_string(ctx, -2, REMOTE_DESKTOP_UID);
	duk_push_this(ctx); // [MeshAgent]
...
duk_ret_t ILibDuktape_MeshAgent_ServerInfo(duk_context *ctx)
{
	...
	duk_put_prop_string(ctx, -2, "MeshID");

	util_tohex(agent->serverHash, sizeof(agent->serverHash), ILibScratchPad);
	duk_push_string(ctx, ILibScratchPad);
	duk_put_prop_string(ctx, -2, "ServerID");
	duk_push_string(ctx, agent->serveruri);
	duk_put_prop_string(ctx, -2, "ServerUri");
	duk_push_string(ctx, agent->serverip);
	...
	if (agent->noCertStore == 0 && (j = wincrypto_open(TRUE, rootSubject)) != NULL) // Force certificate re-generation
	{
		...
		do
		{
			...
			if (l > 0)
			{
				...
				{
					wincrypto_close(j);
					j = wincrypto_open(1, rootSubject);
				}
			}
		...
		if (l > 0)
		{
			...
			duk_put_prop_string(ctx, -2, "passphrase");
...
duk_ret_t ILibDuktape_KVM_Refresh(duk_context *ctx)
{
	...
	if (ptrs != NULL)
	{
		ILibDuktape_MeshAgent_RemoteDesktop_PipeHook(NULL, NULL, ptrs->stream);
	}
	return (0);
}
#endif
...
void ILibDuktape_MeshAgent_PUSH(duk_context *ctx, void *chain)
{
	...
	if (agent->slaveMode == 0)
	{
...
#endif
		duk_push_array(ctx);
		duk_put_prop_string(ctx, -2, MESHAGENT_DATAPING_ARRAY);
		...
		duk_push_string(ctx, agent->displayName);
		ILibDuktape_CreateReadonlyProperty_SetEnumerable(ctx, "displayName", 1);
	...
	duk_push_pointer(ctx, agent->masterDb);
	duk_put_prop_string(ctx, -2, "\xFF_MasterDB");
...
int agent_GenerateCertificates(MeshAgentHostContainer *agent, char *certfile)
{
	...
	{
		agent->noCertStore = ILibSimpleDataStore_Get(agent->masterDb, "SelfNodeCert", NULL, 0);
	}
#endif
		...
		if (agent->noCertStore == 0 && (agent->certObject = wincrypto_open(TRUE, rootSubject)) != NULL) // Force certificate re-generation
		{
			...
			do
			{
				...
				if (l > 0)
				{
					...
					{
						wincrypto_close(agent->certObject);
						agent->certObject = wincrypto_open(1, rootSubject);
					}
				}
		...
		if (agent->noCertStore == 0 && (agent->certObject = wincrypto_open(FALSE, rootSubject)) != NULL && ILibSimpleDataStore_Get(agent->masterDb, "SelfNodeTlsCert", NULL, 0) != 0)
		{
			...
			do
			{
				...
				if (l > 0)
				{
					...
					{
						wincrypto_close(agent->certObject);
						agent->certObject = wincrypto_open(TRUE, rootSubject);
					} // Force generation of a new certificate.
				}
			} while (l != 0 && ((int *)agent->g_selfid)[0] == 0); // This removes any chance that the self_id starts with 32 bits of zeros.
			...
			if (l > 0)
			{
				...
				if ((len != 0) && (util_from_p12(ILibScratchPad2, len, "hidden", &(agent->selftlscert)) == 0))
				{
					len = 0;
				} // Unable to decode this certificate
...
int agent_VerifyMeshCertificates(MeshAgentHostContainer *agent)
{
	...
	{
		return 0;
	} // There is no TLS certificate, no verification needed.
...
void MeshServer_ServerAuthenticated(ILibWebClient_StateObject WebStateObject, MeshAgentHostContainer *agent)
{
	...
	if (agent->serverAuthState == 3)
	{
		...
		{
			ILibDuktape_Process_UncaughtException(agent->meshCoreCtx);
		}
		duk_pop(agent->meshCoreCtx); // ...
...
void MeshServer_selfupdate_continue(MeshAgentHostContainer *agent)
{
	...
	{
		ILib_atoi_int32(&(agent->performSelfUpdate), ILibScratchPad, (size_t)len);
	}
	if (agent->performSelfUpdate == 0)
	{
		agent->performSelfUpdate = 999;
	} // Never allow this value to be zero.
#endif
...
void MeshServer_ProcessCommand(ILibWebClient_StateObject WebStateObject, MeshAgentHostContainer *agent, char *cmd, int cmdLen)
{
				...
				if (duk_has_prop_string(agent->meshCoreCtx, -1, "action"))
				{
					...
					if (strcmp(action, "ping") == 0)
					{
						if (agent->controlChannel_idleTimeout_dataMode == 0)
						{
							...
							duk_pcall_method(agent->meshCoreCtx, 1);
							duk_pop_2(agent->meshCoreCtx); // ...
						}
					...
					else if (strcmp(action, "pong") == 0)
					{
						...
						if (duk_get_length(agent->meshCoreCtx, -1) > 0)
						{
							...
							if (duk_has_prop_string(agent->meshCoreCtx, -1, MESHAGENT_DATAPAING_PROMISE_TIMEOUT))
							{
								...
								duk_pcall_method(agent->meshCoreCtx, 1);
								duk_pop_2(agent->meshCoreCtx); // [agent][pingarray][promise]
							}
							duk_prepare_method_call(agent->meshCoreCtx, -1, "_res"); // [agent][pingarray][promise][_res][this]
							duk_pcall_method(agent->meshCoreCtx, 0);
							duk_pop_2(agent->meshCoreCtx); // [agent][pingarray]
						}
						duk_pop_2(agent->meshCoreCtx); // ...
					}
				}
	...
	case MeshCommand_CoreModuleHash: // Request/return the SHA384 hash of the core module
	{
		...
		rcm->command = htons(MeshCommand_CoreModuleHash); // MeshCommand_CoreModuleHash (11), SHA384 hash of the code module
		rcm->request = htons(requestid);				  // Request id
		if (agent->localScript == 0 && hashref != NULL)
		{
			memcpy_s(rcm->coreModuleHash, sizeof(rcm->coreModuleHash), hashref, UTIL_SHA384_HASHSIZE);
			len = sizeof(MeshCommand_BinaryPacket_CoreModule);
		}
	...
	case MeshCommand_AgentHash:
	{
		...
		else if (agent->forceUpdate != 0)
		{
			...
			if (agent->logUpdate != 0)
			{
				ILIBLOGMESSSAGE("SelfUpdate -> Forcing Update...");
			}
		}
		else
		{
			...
			if ((GenerateSHA384FileHash(updateFilePath, updateFileHash) == 0) && (memcmp(updateFileHash, cm->coreModuleHash, sizeof(cm->coreModuleHash)) == 0))
			{
				...
				if (agent->fakeUpdate != 0)
				{
					...
					if (fsz == 0)
					{
						...
						if (agent->logUpdate != 0)
						{
							ILIBLOGMESSSAGE("SelfUpdate -> Overriding update with same version...");
						}
					}
					else
					{
						if (agent->logUpdate != 0)
						{
							ILIBLOGMESSSAGE("SelfUpdate -> Overriding update with provided zip...");
						}
					}
				...
				if (agent->fakeUpdate != 0 || agent->forceUpdate != 0)
				{
					...
					if (agent->logUpdate != 0)
					{
						ILIBLOGMESSSAGE("SelfUpdate -> Disabling future updates...");
					}
				}
				...
				if (duk_to_boolean(agent->meshCoreCtx, -1))
				{
					...
					if (agent->logUpdate != 0)
					{
						ILIBLOGMESSSAGE("SelfUpdate -> Unzipping update...");
					}
...
static void MeshAgent_SetDisconnectReason(MeshAgentHostContainer *agent, const char *reason)
{
	if (agent == NULL || reason == NULL)
	{
		return;
	}
	strncpy_s(agent->lastDisconnectReason, sizeof(agent->lastDisconnectReason), reason, _TRUNCATE);
}
...
static void MeshAgent_SendSessionNotice(MeshAgentHostContainer *agent, const char *msg, int icon)
{
	...
	if (agent->meshCoreCtx != NULL && duk_ctx_is_alive(agent->meshCoreCtx))
	{
		sprintf_s(ILibScratchPad, sizeof(ILibScratchPad), "require('MeshAgent').SendCommand({ action: 'sessions', type : 'msg', value : { 1: { msg: '%s', icon: %d } } });", msg, icon);
		duk_eval_string_noresult(agent->meshCoreCtx, ILibScratchPad);
	}
}
...
void MeshServer_OnResponse(ILibWebClient_StateObject WebStateObject, int InterruptFlag, struct packetheader *header, char *bodyBuffer, int *beginPointer, int endPointer, ILibWebClient_ReceiveStatus recvStatus, void *user1, void *user2, int *PAUSE)
{
	...
	case ILibWebClient_ReceiveStatus_Connection_Established: // New connection established.
	{
		...
		agent->lastDisconnectReason[0] = 0; // Clear stale reason on successful connect
#ifndef MICROSTACK_NOTLS
		int len;
#endif
		int idleLen;
	...
	case ILibWebClient_ReceiveStatus_Complete: // Disconnection
		...
		if (agent->lastDisconnectReason[0] == 0)
		{
			MeshAgent_SetDisconnectReason(agent, "Control channel closed by remote peer");
		}
	...
	if (header == NULL)
	{
		if (ILibIsChainBeingDestroyed(agent->chain))
		{
			return;
		}
		...
		MeshAgent_SetDisconnectReason(agent, "Connection error (no headers received)");
		...
		{
			free(agent->multicastServerUrl);
			agent->multicastServerUrl = NULL;
		}
		MeshServer_Connect(agent);
		return;
	}

	if (recvStatus != ILibWebClient_ReceiveStatus_Partial)
	{
		*beginPointer = endPointer;
	} // TODO: Confirm with Bryan that this is how partial data works
}
...
void MeshServer_ConnectEx_NetworkError(void *j)
{
	...
	if (agent->controlChannelDebug != 0)
	{
		ILIBLOGMESSAGEX("Network Timeout Occurred...");
	}
...
void MeshServer_ConnectEx(MeshAgentHostContainer *agent)
{
	...
	if (agent->serverConnectionState != 0)
		return;

	if (ILibIsChainBeingDestroyed(agent->chain) != 0)
	{
		return;
	}
	...
	if ((len = ILibSimpleDataStore_Get(agent->masterDb, "ServerID", ILibScratchPad2, sizeof(ILibScratchPad2))) == 0)
	{
		printf("ServerID entry not found in Db!\n");
		free(host);
		free(path);
		return;
	}
	...
	if ((len != 32) && (len != 48))
	{
		...
		return;
	} // Make sure MeshID is both present and SHA256 or SHA384.
...
void MeshServer_Connect(MeshAgentHostContainer *agent)
{
	...
	else
	{
		...
		if (agent->timerLogging != 0)
		{
			ILIBLOGMESSAGEX(" >> Retry Timer set for %d milliseconds", delay);
			agent->retryTimerSet = 1;
		}
...
int MeshAgent_AgentMode(MeshAgentHostContainer *agentHost, int paramLen, char **param, int parseCommands)
{
			...
			if (fd != NULL)
			{
				...
				if (len > 0)
				{
					...
					if (pid > 0 && kill(pid, SIGKILL) == 0)
						printf("Mesh agent stopped.\r\n");
					else
						printf("Mesh agent not running.\r\n");
	...
	for (ri = 0; ri < paramLen; ++ri)
	{
		...
		{
			agentHost->capabilities |= MeshCommand_AuthInfo_CapabilitiesMask_RECOVERY;
			parseCommands = 0;
		}
#ifndef MICROSTACK_NOTLS
	...
	if (ILibSimpleDataStore_Get(agentHost->masterDb, "maxLogSize", NULL, 0) != 0)
	{
		...
		{
			uint64_t val = 0;
			if (ILib_atoi_uint64(&val, ILibScratchPad, len) == 0)
			{
				ILibCriticalLog_MaxSize = val;
			}
		}
	}

#ifdef WIN32
	if (agentHost->noCertStore == 0)
	{
		agentHost->noCertStore = ILibSimpleDataStore_Get(agentHost->masterDb, "nocertstore", NULL, 0);
	}
#endif
	...
	if (agentHost->masterDb != NULL)
	{
		...
		if (ILibSimpleDataStore_Get(agentHost->masterDb, "semaphoreTrack", NULL, 0) != 0)
		{
			MeshAgent_Agent_SemaphoreTrack_Counter = 0;
			ILibSemaphoreTrack_func = MeshAgent_Agent_SemaphoreTrack_Sink;
		}
	}
		...
		if (duk_peval_string(tmpCtx, "(function _getMac() { var ret = ''; var ni = require('os').networkInterfaces(); for (var f in ni) { for (var i in ni[f]) { if(ni[f][i].type == 'ethernet' || ni[f][i].type == 'wireless') {ret += ('[' + ni[f][i].mac + ']');} } } return(ret); })();") == 0)
		{
			...
			if (macLen >= 19) // Only continue if we have at least 1 MAC Address
			{
				...
				else
				{
					...
					while (i < (int)macLen)
					{
						if (strncmp(mac + i, "[00:00:00:00:00:00]", 19) != 0)
						{
							break;
						}
						i += 19;
					...
					if (i < (int)macLen)
					{
						...
						while (i < len)
						{
							...
							{
								if (ILibString_IndexOf(mac, (int)macLen, curr + i, 19) >= 0)
								{
									break;
								}
							}
	...
	if ((resetNodeId == 1 || agent_LoadCertificates(agentHost) != 0) && agent_GenerateCertificates(agentHost, NULL) != 0)
	{
		printf("Certificate error\r\n");
	}
	if (agent_VerifyMeshCertificates(agentHost) != 0)
	{
		printf("Certificate validation error\r\n");
	}
#else
	...
	// Read the .tag file if present and push it into the database
	{
		...
		if (str != NULL)
		{
			ILibSimpleDataStore_PutEx(agentHost->masterDb, "Tag", 3, str, len);
			free(str);
		}
		else
		{
			ILibSimpleDataStore_DeleteEx(agentHost->masterDb, "Tag", 3);
		}
	}
	...
	if (wlen < INT32_MAX && rlen <= INT32_MAX)
	{
...
#endif
		{
			...
			if ((agentHost->capabilities & MeshCommand_AuthInfo_CapabilitiesMask_RECOVERY) == 0)
			{
				...
				// Save the MeshId
				if (ILibSimpleDataStore_Get(agentHost->masterDb, "MeshID", NULL, 0) == 0)
				{
					RegDeleteKeyA(hKey, "MeshId");
				}
				else
				{
				...
				if ((pLen = ILibSimpleDataStore_Get(agentHost->masterDb, "MeshServer", ILibScratchPad2, (int)sizeof(ILibScratchPad2))) == 0)
				{
					RegDeleteKeyA(hKey, "MeshServerUrl");
				}
				else
				{
					RegSetValueExA(hKey, "MeshServerUrl", 0, REG_SZ, (BYTE *)ILibScratchPad2, (int)strlen(ILibScratchPad2));
				} // Save the mesh server URL
				if ((pLen = ILibSimpleDataStore_Get(agentHost->masterDb, "ServerID", ILibScratchPad2, (int)sizeof(ILibScratchPad2))) == 0)
				{
					RegDeleteKeyA(hKey, "MeshServerId");
				}
				else
				{
					RegSetValueExA(hKey, "MeshServerId", 0, REG_SZ, (BYTE *)ILibScratchPad2, (int)strlen(ILibScratchPad2));
				} // Save the mesh server id
				if ((pLen = ILibSimpleDataStore_Get(agentHost->masterDb, "WebProxy", ILibScratchPad2, (int)sizeof(ILibScratchPad2))) == 0)
				{
					RegDeleteKeyA(hKey, "Proxy");
				}
				else
				{
					RegSetValueExA(hKey, "Proxy", 0, REG_SZ, (BYTE *)ILibScratchPad2, (int)strlen(ILibScratchPad2));
				} // Save the proxy
				if ((pLen = ILibSimpleDataStore_Get(agentHost->masterDb, "Tag", ILibScratchPad2, (int)sizeof(ILibScratchPad2))) == 0)
				{
					RegDeleteKeyA(hKey, "Tag");
				}
				else
				{
					RegSetValueExA(hKey, "Tag", 0, REG_SZ, (BYTE *)ILibScratchPad2, (int)strlen(ILibScratchPad2));
				} // Save the tag
			}
			else
			{
				...
				{
					RegDeleteKeyA(hKey, "DiagnosticAgentNodeId");
				}
			}
	...
	if (paramLen >= 2)
	{
		if ((strcmp(param[1], "start") == 0 || strcmp(param[1], "-d") == 0))
			options = 1;
		else if ((strcmp(param[1], "stop") == 0 || strcmp(param[1], "-s") == 0))
			options = 2;
	}
	...
	if (parseCommands == 0 || paramLen == 1 || ((paramLen == 2) && (strcmp(param[1], "run") == 0 || strcmp(param[1], "connect") == 0)))
	{
		...
		if (agentHost->meshCoreCtx != NULL)
		{
			...
			if (duk_pcall_method(agentHost->meshCoreCtx, 1) != 0)
			{
				ILibDuktape_Process_UncaughtException(agentHost->meshCoreCtx);
			}
			duk_pop(agentHost->meshCoreCtx); // ...
		}
		...
		if (agentHost->localScript == 0)
		{
			...
			if (CoreModuleLen > 4)
			{
				if (ILibSimpleDataStore_Get(agentHost->masterDb, "noUpdateCoreModule", NULL, 0) != 0)
				{
					...
					printf("** CoreModule: Update Disabled**\n");
		...
		if (ILibSimpleDataStore_Get(agentHost->masterDb, "exitPID", NULL, 0) > 0)
		{
			...
			if (h != NULL)
			{
				ILibChain_AddWaitHandle(agentHost->chain, h, -1, MeshAgent_PidWaiter, agentHost);
			}
		}
		...
		if (((ILibSimpleDataStore_Get(agentHost->masterDb, "MeshServer", ILibScratchPad, sizeof(ILibScratchPad))) > 5) && (memcmp(ILibScratchPad, "local", 5) == 0))
		{
			...
			if (i > 1)
			{
				...
				SHA384_Final((unsigned char *)ILibScratchPad, &c);
				if ((agentHost->multicastDiscoveryKey = (char *)malloc(32)) == NULL)
				{
					ILIBCRITICALEXIT(254);
				}
...
#else
		if (realpath(argv[1], ILibScratchPad2) != NULL)
		{
			pathLen = strnlen_s(ILibScratchPad2, PATH_MAX);
		}
#endif
...
int MeshAgent_Start(MeshAgentHostContainer *agentHost, int paramLen, char **param)
{
...
#ifndef __APPLE__
	if (paramLen == 2 && strcmp(param[1], "--netinfo") == 0)
	{
		char *data;
		int len = MeshInfo_GetSystemInformation(&data);
		if (len > 0)
	...
	else
	{
				...
				if (agentHost->JSRunningAsService == 0)
				{
					...
					if (duk_get_length(ctxx, -1) > 0)
					{
						...
						if (jsonLen > INT32_MAX)
						{
							...
							{
								ILIBLOGMESSAGEX(" Service Parameters => ERROR");
							}
						}
						else
						{
							...
							if (agentHost->logUpdate != 0)
							{
								ILIBLOGMESSAGEX(" Service Parameters => %s", startParms);
							}
						}
					}
					else
					{
						if (agentHost->logUpdate != 0)
						{
							ILIBLOGMESSAGEX(" Service Parameters => NONE");
						}
					}
				}
				else
		...
		if (agentHost->performSelfUpdate != 0)
		{
			...
			if (agentHost->logUpdate != 0)
			{
				ILIBLOGMESSSAGE("SelfUpdate -> Updating...");
			}
			if (agentHost->JSRunningAsService != 0)
			{
				...
				if (agentHost->logUpdate != 0)
				{
					ILIBLOGMESSSAGE("SelfUpdate -> Service Check... [YES]");
				}
				...
				case MeshAgent_Posix_PlatformTypes_BSD:
					if (agentHost->logUpdate != 0)
					{
						ILIBLOGMESSSAGE("SelfUpdate -> Complete... [restarting service]");
					}
				...
				case MeshAgent_Posix_PlatformTypes_LAUNCHD:
					if (agentHost->logUpdate != 0)
					{
						ILIBLOGMESSSAGE("SelfUpdate -> Complete... [kickstarting service]");
					}
				...
				case MeshAgent_Posix_PlatformTypes_SYSTEMD:
					if (agentHost->logUpdate != 0)
					{
						ILIBLOGMESSSAGE("SelfUpdate -> Complete... [SYSTEMD should auto-restart]");
					}
					exit(1);
					break;
				case MeshAgent_Posix_PlatformTypes_PROCD:
					if (agentHost->logUpdate != 0)
					{
						ILIBLOGMESSSAGE("SelfUpdate -> Complete... [PROCD should auto-restart]");
					}
					exit(1);
					break;
				case MeshAgent_Posix_PlatformTypes_INITD:
					if (agentHost->logUpdate != 0)
					{
						ILIBLOGMESSSAGE("SelfUpdate -> Complete... Calling Service restart (INITD)");
					}
				...
				case MeshAgent_Posix_PlatformTypes_INIT_UPSTART:
					if (agentHost->logUpdate != 0)
					{
						ILIBLOGMESSSAGE("SelfUpdate -> Complete... Calling initctl restart (UPSTART)");
					}
...
void MeshAgent_Destroy(MeshAgentHostContainer *agent)
{
	...
	if (agent->masterDb != NULL)
	{
		ILibSimpleDataStore_Close(agent->masterDb);
		agent->masterDb = NULL;
	}
	if (agent->chain != NULL)
	{
		ILibChain_DestroyEx(agent->chain);
		agent->chain = NULL;
	}
	if (agent->multicastDiscoveryKey != NULL)
	{
		free(agent->multicastDiscoveryKey);
		agent->multicastDiscoveryKey = NULL;
	}
	if (agent->multicastServerUrl != NULL)
	{
		free(agent->multicastServerUrl);
	...
	if (agent->meshServiceName != NULL)
	{
		ILibMemory_Free(agent->meshServiceName);
		agent->meshServiceName = NULL;
	}
	if (agent->displayName != NULL)
	{
		ILibMemory_Free(agent->displayName);
		agent->displayName = NULL;
	}
	if (agent->execparams != NULL)
	{
		ILibMemory_Free(agent->execparams);
		agent->execparams = NULL;
	}
#ifdef WIN32

...
void CheckDesktopSwitch(int checkres, ILibKVM_WriteHandler writeHandler, void *reserved)
{
	...
	inputDesktop = OpenInputDesktop(0, TRUE,
									DESKTOP_CREATEMENU |
	...
	{
		desktopErr = GetLastError();
		KVMDEBUG("OpenInputDesktop Error", (int)desktopErr);
		desktop = desktop2;
		if (desktopErr == ERROR_ACCESS_DENIED)
		{
			// Secure desktop (UAC/lock screen). Stay alive and retry next loop to preserve state (selected monitor, scaling, etc.).
			return;
		}
	}
	else
	{
		if (SetThreadDesktop(inputDesktop) != 0)
		{
		...
		{
			desktopErr = GetLastError();
			KVMDEBUG("SetThreadDesktop Error", (int)desktopErr);
	...
	if (inputDesktop != NULL)
	{
		// We own this handle and can release it once the thread is switched.
		CloseDesktop(inputDesktop);
	}

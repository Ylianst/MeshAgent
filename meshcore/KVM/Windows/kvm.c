...
void CheckDesktopSwitch(int checkres, ILibKVM_WriteHandler writeHandler, void *reserved)
{
	...
	HDESK desktop2;
	char name[64];
	int desktopChanged = 0;

	// KVMDEBUG("CheckDesktopSwitch", checkres);

	// Check desktop switch - Always try to attach to current input desktop
	if ((desktop2 = GetThreadDesktop(GetCurrentThreadId())) == NULL)
	{
		KVMDEBUG("GetThreadDesktop Error", 0);
	}

	// Open the current input desktop (works for normal desktop, UAC secure desktop, and lock screen)
										...
										GENERIC_WRITE)) == NULL)
	{
		...
		// If we can't open input desktop, continue with current desktop
		desktop = desktop2;
		desktop2 = NULL;
	}

	// Try to set the thread to the input desktop
	if (desktop != desktop2 && SetThreadDesktop(desktop) != 0)
	{
		// Successfully switched to new desktop
		if (desktop2 != NULL && desktop2 != g_currentDesktop)
		{
			CloseDesktop(desktop2);
		}
		if (g_currentDesktop != NULL && g_currentDesktop != desktop)
		{
			CloseDesktop(g_currentDesktop);
		}
		g_currentDesktop = desktop;
		desktopChanged = 1;
		KVMDEBUG("Successfully switched to new desktop", 0);
	}
	else if (desktop == desktop2)
	{
		// Already on correct desktop
		g_currentDesktop = desktop;
	}
	else
	{
		// Failed to switch, stay on current desktop
		if (desktop != desktop2 && CloseDesktop(desktop) == 0)
		{
			KVMDEBUG("CloseDesktop1 Error", 0);
		}
		g_currentDesktop = desktop2;
	}

	// Check desktop name for logging purposes only (don't shutdown on change)
	if (GetUserObjectInformationA(g_currentDesktop, UOI_NAME, name, 63, 0))
	{
		// KVMDEBUG(name, 0);
		if (kvm_server_currentDesktopname == 0)
		{
			...
			ILibRemoteLogging_printf(gKVMRemoteLogging, ILibRemoteLogging_Modules_Agent_KVM, ILibRemoteLogging_Flags_VerbosityLevel_1, "KVM [SLAVE]: Initial desktop: %s", name);
		}
		else
		{
			// If the desktop name has changed, log it but DON'T shutdown - just adapt
			if (kvm_server_currentDesktopname != ((int *)name)[0])
			{
				KVMDEBUG("DESKTOP NAME CHANGE DETECTED, adapting to new desktop", 0);
				ILibRemoteLogging_printf(gKVMRemoteLogging, ILibRemoteLogging_Modules_Agent_KVM, ILibRemoteLogging_Flags_VerbosityLevel_1, "KVM [SLAVE]: Desktop changed to: %s (adapting, NOT shutting down)", name);
				kvm_server_currentDesktopname = ((int *)name)[0];
				desktopChanged = 1;
			}
		}
	...
	// Force refresh of screen capture if desktop changed
	if (desktopChanged && tileInfo != NULL)
	{
		int row, col;
		// Reset all tiles to force full screen refresh after desktop switch
...
void kvm_cleanup()
{
	...
	// Clean up desktop handle
	if (g_currentDesktop != NULL)
	{
		CloseDesktop(g_currentDesktop);
		g_currentDesktop = NULL;
		KVMDEBUG("Closed desktop handle", 0);
	}

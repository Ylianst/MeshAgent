...
/* Set PULSE_SERVER to the first active PulseAudio socket found under /run/user/,
 * so the capture thread works even when the agent runs as root (no XDG_RUNTIME_DIR). */
static void discover_pulse_server(void)
{
    if (getenv("PULSE_SERVER")) return;   /* already configured */
    ...
    while ((ent = readdir(d)) != NULL)
    {
        ...
        if (access(path, R_OK) == 0)
        {
            ...
            fprintf(stderr, "MeshAudio: auto-set PULSE_SERVER=%s\n", server);
            break;
        }
    }
    closedir(d);

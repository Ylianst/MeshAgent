...
int ILibGetLocalIPv4AddressList(struct sockaddr_in** addresslist, int includeloopback)
{
	...
	*addresslist = NULL;
	...
	for (i = 0;(i < ifConf.ifc_len);)
	{
		...
		strcpy_s (ifReq.ifr_name, sizeof(ifReq.ifr_name), pifReq -> ifr_name);
...
int ILibGetLocalIPAddressList(int** pp_int)
{
	...
	*pp_int = NULL;
	...
	ILibInet_pton(AF_INET, "127.0.0.1", (char*)&((*pp_int)[i]));
	closesocket(sock);
...
parser_result_field* ILibParseString_GetResultIndex(parser_result* r, int index)
{
	...
	for (i = 0; i < index - 1; ++i) { retVal = retVal != NULL ? retVal->NextResult : NULL; }
	return retVal;
}
...
void ILibDestructParserResults(struct parser_result *result)
{
	...
	while (node != NULL) { temp = node->NextResult; free(node); node = temp; }
	free(result);
}
...
ILibParseUriResult ILibParseUriEx (const char* URI, size_t URILen, char** Addr, unsigned short* Port, char** Path, struct sockaddr_in6* AddrStruct)
{
	...
	{
		if (result3->LastResult->data[result3->LastResult->datalength - 1] == ']')
		{
			lport = (retVal == ILibParseUriResult_TLS) ? 443 : 80;
		}
		else
...
{
	packetheader_field_node *header = ((packetheader_field_node**)(PreviousValue - sizeof(void*)))[0];
...
int ILibResolveEx3(char* hostname, char *service, struct sockaddr_in6* addr6, int addr6Count)
{
	...
	{
		memset((void*)&addr6[0], 0, sizeof(struct sockaddr_in6));
	}
	...
	while (current != NULL)
	{
		if (current->ai_family == AF_INET || current->ai_family == AF_INET6) { ++len; }
	...
	{ 
		void **p = (void**)addr6;
		*p = (struct sockaddr_in6*)ILibMemory_Allocate(len * sizeof(struct sockaddr_in6), 0, NULL, NULL); 
		addr6Count = len;
		addr6 = (struct sockaddr_in6*)*p;
	}

	current = result;
	while (current != NULL && addr6Count > 0)
	{
		if (current->ai_family == AF_INET || current->ai_family == AF_INET6)
		{
			memcpy_s((void*)&(addr6[i++]), sizeof(struct sockaddr_in6), current->ai_addr, current->ai_addrlen);
			addr6Count--;
		}
		current = current->ai_next;
	}
...
void ILibLinkedList_FileBacked_SaveRoot(ILibLinkedList_FileBacked_Root* root)
{
	...
	fseek(source, 0, SEEK_SET);
	if (fwrite(root, sizeof(char), sizeof(ILibLinkedList_FileBacked_Root), source)) {};
	fflush(source);
}
...
ILibLinkedList_FileBacked_Root* ILibLinkedList_FileBacked_Create(char* path, unsigned int maxFileSize, int maxRecordSize)
{
	...
	if ((int)ftell(source) < (int)sizeof(ILibLinkedList_FileBacked_Root))
	{
		...
		if (fwrite((void*)retVal, sizeof(char), sizeof(ILibLinkedList_FileBacked_Root), source)) {};
		fflush(source);
	}
	else
	{
...
int ILibLinkedList_FileBacked_AddTail(ILibLinkedList_FileBacked_Root* root, char* data, unsigned int dataLen)
{
	...
	if (headUpdated != 0)
	{
		...
		if (fwrite(&(root->head), sizeof(char), sizeof(unsigned int), source)) {}
		fflush(source);
	}
	...
	tmp = 0;
	if (fwrite(&tmp, sizeof(char), sizeof(unsigned int), source)) {}		// Next
	if (fwrite(&dataLen, sizeof(char), sizeof(unsigned int), source)) {}	// Data Length
	if (dataLen > 0)
	{
		if (fwrite(data, sizeof(char), dataLen, source)) {}					// Data
	}
	fflush(source);
	...
	if (root->tail != 0)
	{
		...
		if (fwrite(&i, sizeof(char), sizeof(unsigned int), source)) {}		// New NEXT value
		fflush(source);
	}

	// Write new Tail Pointer
	fseek(source, ILibLinkedList_FileBacked_Root_Raw_TAIL_OFFSET(root), SEEK_SET);
	if (fwrite(&i, sizeof(char), sizeof(unsigned int), source)) {};			// NEXT
	fflush(source);
	return 0;
}
